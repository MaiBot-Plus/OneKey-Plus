...
class MaiBotManager:
        ...
        try:
            ...
            if repo_url.startswith("https://github.com/"):
                ...
                try:
                    ...
                    restore_success, restore_output = self.run_command_with_env(restore_url_cmd, cwd=repo_path, env=env, show_output=False)
                    
                    if not restore_success:
                        stderr = restore_output.get('stderr', '') if isinstance(restore_output, dict) else str(restore_output)
    ...
    def _update_without_token(self, service: dict, repo_path: Path) -> bool:
        ...
        success, output = self.run_command_with_env(['git', 'pull'], cwd=repo_path, env=env, show_output=False)
        if not success:
